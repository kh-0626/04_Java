...
public class ObjectService {
	...
	public void outputMember() {
		...
		try {
			// 출력할 Member 객체 생성
			Member member = new Member("member111222333", "1q2w3e4r1", "짱구");
	...
	public void outputMemberList() {
		...
		try {
			...
			try {
				if(oos !=null) oos.close(); // 보조스트림 close 시 기반 스트림도 같이 close
			} catch (IOException e) { 
				e.printStackTrace();
			} 
		}
	...
	public void inputMemberList() {
		...
		try {
			...
			try {
				...
				if(ois !=null) ois.close(); // 보조스트림 close 시 기반 스트림도 같이 close
			} catch (IOException e) { 
				e.printStackTrace();
			}
		}
